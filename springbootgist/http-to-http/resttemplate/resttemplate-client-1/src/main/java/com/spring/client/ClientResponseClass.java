...
public class ClientResponseClass implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        Link link = getLink(ROOT_URI,"fruits");
        findAllFruit(link);
        Resource<Fruit> apple = addFruit(link);
        Link orderLink = getLink(ROOT_URI, "fruitOrderMenus");
...
//        queryOrders(orderLink);
    ...
    private Link getLink(URI uri,String rel){
        ResponseEntity<Resource<Link>> rootresp = restTemplate.exchange(uri, HttpMethod.GET,null,
                new ParameterizedTypeReference<Resource<Link>>() {});
        Link link = rootresp.getBody().getLink(rel);
        log.info("{}",rootresp.getBody());
        log.info("Link:{}",link);
        return link;
    }
    ...
    private void addOrder(Link link, Resource<Fruit> fruitResource) {
        FruitOrderMenu newOrder = FruitOrderMenu.builder()
                .customer("Li Lei")
                .states(OrderMenuStates.BOOK)
                .build();
        RequestEntity<?> req =
        ...
        Link lists = order.getLink("lists");
        req = RequestEntity.post(lists.getTemplate().expand()).body(Collections.singletonMap("_links", fruitResource.getLink("self")));
